...
import { checkCampaignConversions, checkAllCampaignsConversions } from '../services/conversions.js';
...
// POST /api/campaigns/:id/check-conversions — verificar conversiones de UNA campaña
// Query param: ?days=30 (ventana de atribución)
router.post('/:id/check-conversions', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, data: { ...result, attribution_days: attributionDays } });
  } catch (err) {
...
// POST /api/campaigns/check-all-conversions — verificar TODAS las campañas recientes
// Llamado por el botón del dashboard y por el cron nocturno
// Query param: ?days=30 (ventana de atribución)
router.post('/check-all-conversions', async (req, res) => {
  if (!process.env.WOOCOMMERCE_URL) {
    return res.status(400).json({ success: false, error: 'WooCommerce no está configurado' });
  }
  ...
  try {
    const result = await checkAllCampaignsConversions(attributionDays);
    res.json({ success: true, data: result });
  } catch (err) {
    console.error('[Campaigns] check-all-conversions error:', err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});
